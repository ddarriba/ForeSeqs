...
complex complv(double re, double im);
complex cdiv(complex a, complex b);

int Utils::abyx(double a, double x[], int n) {
	int i;
	for (i = 0; i < n; x[i] *= a, i++)
		;
	return (0);
}
int Utils::xtoy(double x[], double y[], int n) {
	int i;
	for (i = 0; i < n; y[i] = x[i], i++)
		;
	return (0);
}
int Utils::matinv(double x[], int n, int m, double space[]) {
	/* x[n*m]  ... m>=n
	 */
	register int i, j, k;
	int *irow = (int*) space;
...
int Utils::eigen(int job, double A[], int n, double rr[], double ri[],
			...
			if (l == en - 1) { /* two roots found */
				...
				if (q < 0) { /* complex pair */
					...
					if (job) {
						...
						for (j = en - 1; j < n; j++) {
							z = mat[pos(en - 1, j, n)];
							mat[pos(en - 1, j, n)] = q * z
									+ p * mat[pos(en, j, n)];
							mat[pos(en, j, n)] = q * mat[pos(en, j, n)] - p * z;
						}
						for (i = 0; i <= en; i++) {
							z = mat[pos(i, en - 1, n)];
							mat[pos(i, en - 1, n)] = q * z
									+ p * mat[pos(i, en, n)];
		...
		for (en = n - 1; en >= 0; en--) {
			...
			if ((q = vali[en]) < 0) { /* complex vector */
				...
				if (fabs(mat[pos(en, en - 1, n)])
					...
					v = cdiv(complv(0.0, -mat[pos(en - 1, en, n)]),
							complv(mat[pos(en - 1, en - 1, n)] - p, q));
				...
				for (i = en - 2; i >= 0; i--) {
					...
					if (vali[i] < 0) {
						...
						m = i;
						if (vali[i] == 0) {
							v = cdiv(complv(-ra, -sa), complv(w, q));
							...
							v = cdiv(
									complv(x * r - z * ra + q * sa,
											x * s - z * sa - q * ra), v);
							mat[pos(i, en - 1, n)] = v.re;
							mat[pos(i, en, n)] = v.im;
							if (fabs(x) > fabs(z) + fabs(q)) {
								mat[pos(i + 1, en - 1, n)] = (-ra
										- w * mat[pos(i, en - 1, n)]
										+ q * mat[pos(i, en, n)]) / x;
								mat[pos(i + 1, en, n)] = (-sa
										- w * mat[pos(i, en, n)]
